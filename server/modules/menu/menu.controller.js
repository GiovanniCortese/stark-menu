...
exports.getMenuBySlug = async (req, res) => {
  try {
    ...
    // ✅ FIX: nel DB la colonna è p.categoria (NON categoria_id)
    const menuQuery = `
      SELECT 
        p.*, 
        ...
        c.is_pizzeria
      FROM prodotti p
      JOIN categorie c ON p.categoria = c.id
...
exports.getAdminMenu = async (req, res) => {
  try {
    ...
    // ✅ FIX: p.categoria (NON categoria_id)
    const menuQuery = `
      SELECT 
        p.*, 
        c.nome as categoria_nome, 
        c.ordine_visualizzazione
      FROM prodotti p
      LEFT JOIN categorie c ON p.categoria = c.id
...
exports.getCategorie = async (req, res) => {
  try {
    const r = await pool.query(
      "SELECT * FROM categorie WHERE ristorante_id = $1 ORDER BY ordine_visualizzazione ASC",
...
exports.saveProdotto = async (req, res) => {
  ...
  try {
    // ✅ Compatibilità: accetta categoria_id (vecchio frontend) o categoria (nuovo/DB reale)
    const {
      id,
      ristorante_id,
      categoria_id,
    ...
    const categoriaFinal = categoria_id ?? categoria; // <-- usa quello che arriva
    ...
    if (id) {
      // ✅ FIX: aggiorna colonna "categoria" (NON categoria_id)
      await client.query(
        `UPDATE prodotti 
         SET nome=$1, descrizione=$2, prezzo=$3, categoria=$4, foto_url=$5, varianti=$6, allergeni=$7 
         WHERE id=$8`,
        [nome, descrizione, prezzo, categoriaFinal, foto_url, varJson, allJson, id]
      );
    } else {
      // ✅ FIX: inserisce colonna "categoria" (NON categoria_id)
      await client.query(
        `INSERT INTO prodotti (ristorante_id, categoria, nome, descrizione, prezzo, foto_url, varianti, allergeni) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [ristorante_id, categoriaFinal, nome, descrizione, prezzo, foto_url, varJson, allJson]
      );
    }

    res.json({ success: true });
  } catch (e) {
    console.error("saveProdotto error:", e);
...
exports.translateMenu = async (req, res) => {
  ...
  try {
    const menu = await client.query(
      "SELECT id, nome, descrizione, ingredienti_base FROM prodotti WHERE ristorante_id = $1",
      [ristorante_id]
    );
...
Ecco i dati: ${JSON.stringify(itemsToTranslate)}`;
    ...
    for (const t of translations) {
      await client.query(
        `UPDATE prodotti 
         SET traduzioni = jsonb_set(COALESCE(traduzioni, '{}'), '{${lang}}', $1) 
         WHERE id = $2`,
