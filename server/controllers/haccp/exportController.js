...
exports.exportGeneric = async (req, res) => {
    try {
        ...
        // --- MATRICE TEMPERATURE (Vista Calendario) ---
        if (tipo === 'temperature_matrix') {
            ...
            if (format === 'pdf') {
                ...
                // EXCEL CON INTESTAZIONE
                const wb = xlsx.utils.book_new();
                
                // Creiamo un array di array per l'intero foglio
                const sheetData = [
                    [aziendaInfo.nome], // Riga 1: Nome Azienda
                    [aziendaInfo.dati_fiscali || ""], // Riga 2: P.IVA
                    [titoloReport], // Riga 3: Titolo e Mese
                    [""], // Riga 4: Vuota
                    tableHeaders, // Riga 5: Intestazioni Tabella
                    ...tableRows // Righe Dati
                ];
