...
app.get('/api/haccp/merci/:ristorante_id', async (req, res) => {
    try {
        ...
        if (start && end) {
            sql += " AND data_ricezione >= $2 AND data_ricezione <= $3 ORDER BY data_ricezione ASC";
            params.push(start, end);
        } else {
            sql += " AND data_ricezione >= NOW() - INTERVAL '7 days' ORDER BY data_ricezione DESC";
        }
...
app.get('/api/haccp/pulizie/:ristorante_id', async (req, res) => {
    try {
        ...
        if (start && end) {
            // Modalità Calendario (Mese specifico)
            sql += " AND data_ora >= $2 AND data_ora <= $3 ORDER BY data_ora ASC";
            params.push(start, end);
        } else {
            // Modalità Tabella (Ultimi 7 giorni)
            sql += " AND data_ora >= NOW() - INTERVAL '7 days' ORDER BY data_ora DESC";
        }
...
app.get('/api/haccp/labels/storico/:ristorante_id', async (req, res) => {
    try {
        ...
        if (start && end) {
             // Modalità Calendario
             sql += " AND data_produzione >= $2 AND data_produzione <= $3 ORDER BY data_produzione ASC";
             params.push(start, end);
        } else {
             // Modalità Tabella
             sql += " AND data_produzione >= NOW() - INTERVAL '7 days' ORDER BY data_produzione DESC";
        }
