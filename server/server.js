// server/server.js - VERSIONE V19 (SUPPORTO BAR VS CUCINA) 🍹
...
const initDb = async () => {
    ...
    try {
        ...
        await client.query(`
            ...
            CREATE TABLE IF NOT EXISTS ordini (id SERIAL PRIMARY KEY, ristorante_id INTEGER REFERENCES ristoranti(id), tavolo TEXT, stato TEXT DEFAULT 'in attesa', data_ora TIMESTAMP DEFAULT CURRENT_TIMESTAMP, prodotti TEXT, totale REAL, dettagli TEXT);
        `);

        // AGGIUNTA COLONNA PER IL BAR (AUTO-REPAIR)
        await client.query(`
            DO $$ 
            BEGIN 
                -- Aggiunge is_bar alle categorie se non c'è
        ...
        console.log("✅ Database pronto (Supporto Bar Attivo).");
        return true;
...
app.get('/api/menu/:slug', async (req, res) => {
    try {
        ...
        // JOIN per prendere info categoria incluso is_bar
        const query = `
            SELECT p.*, 
...
// 2. CREA CATEGORIA (Supporta is_bar)
app.post('/api/categorie', async (req, res) => { 
    try { 
        const { nome, ristorante_id, descrizione, is_bar } = req.body;
        ...
        const r = await pool.query('INSERT INTO categorie (nome, posizione, ristorante_id, descrizione, is_bar) VALUES ($1, $2, $3, $4, $5) RETURNING *', [nome, next, ristorante_id, descrizione||"", is_bar || false]); 
        res.json(r.rows[0]); 
    } catch (e) { res.status(500).json({error:"Err"}); } 
});

// 3. UPDATE CATEGORIA (Supporta is_bar)
app.put('/api/categorie/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { nome, descrizione, is_bar } = req.body; // is_bar arriva dal frontend
        ...
        if (oldCat.rows.length > 0) {
            ...
            await pool.query('UPDATE categorie SET nome = $1, descrizione = $2, is_bar = $3 WHERE id = $4', [nome, descrizione || "", is_bar, id]);
    ...
    } catch (err) { res.status(500).json({ error: "Errore update" }); }
});

// ALTRE API STANDARD (Invariate ma incluse per completezza)
...
app.post('/api/ordine', async (req, res) => { try { console.log("➡️ Ordine:", req.body); const { ristorante_id, tavolo, prodotti, totale } = req.body; const prodottiStr = JSON.stringify(prodotti); await pool.query('INSERT INTO ordini (ristorante_id, tavolo, prodotti, totale) VALUES ($1, $2, $3, $4)', [ristorante_id, tavolo, prodottiStr, totale]); res.json({ success: true }); } catch (e) { res.status(500).json({error: "Err"}); } });
app.get('/api/polling/:ristorante_id', async (req, res) => { try { const r = await pool.query("SELECT * FROM ordini WHERE ristorante_id = $1 AND (stato = 'in attesa' OR stato = 'in_attesa') ORDER BY id ASC", [req.params.ristorante_id]); const ordini = r.rows.map(o => { let parsed = []; try { parsed = JSON.parse(o.prodotti); } catch (e) { parsed = [{nome: o.prodotti}]; } return { ...o, prodotti: parsed }; }); res.json({ nuovi_ordini: ordini }); } catch (e) { res.status(500).send("Err"); } });
...
initDb().then((ready) => { if (ready) app.listen(port, () => console.log(`🚀 SERVER V19 (BAR) - Porta ${port}`)); });