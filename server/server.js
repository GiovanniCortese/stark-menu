// server/server.js - VERSIONE V24 (FIX JSON ERROR & DEBUGGING) 🛡️
...
const initDb = async () => {
    ...
    try {
        ...
        console.log("✅ Database pronto V24.");
        return true;
...
app.get('/api/polling/:ristorante_id', async (req, res) => { 
    try { 
        ...
        const ordini = r.rows.map(o => {
            ...
            if (o.prodotti) {
                try {
                    ...
                    console.error(`JSON Error ID ${o.id}:`, e);
                    parsed = [{ nome: "⚠️ Errore Dati" }];
                }
            } else if (o.dettagli) {
        ...
        console.error("Polling DB Error:", e);
        // IMPORTANTE: Inviamo JSON anche in errore, non stringa!
        res.status(500).json({ error: "Errore Polling", details: e.message }); 
    } 
});
...
app.post('/api/cassa/paga-tavolo', async (req, res) => {
    try {
        ...
        console.error("Error Paga Tavolo:", e);
        res.status(500).json({ error: "Errore Pagamento" }); 
    }
});

// 4. STORICO ORDINI (FIXED)
app.get('/api/cassa/storico/:ristorante_id', async (req, res) => {
    try {
        // Usa un LIMIT per non far esplodere tutto se ci sono 10000 ordini
        const r = await pool.query(
            "SELECT * FROM ordini WHERE ristorante_id = $1 AND stato = 'pagato' ORDER BY data_ora DESC LIMIT 200", 
            [req.params.ristorante_id]
        );
        ...
        const ordini = r.rows.map(o => {
            let parsed = [];
            try { 
                parsed = JSON.parse(o.prodotti || "[]"); 
            } catch (e) { 
                parsed = [{ nome: "Errore Dati Storico" }]; 
            }
            return { ...o, prodotti: parsed };
        });
        ...
        console.error("Error Storico:", e);
        // IMPORTANTE: Risposta JSON
...
app.post('/api/ordine', async (req, res) => { 
    try { 
        ...
        console.error("Error New Order:", e);
        res.status(500).json({error: "Errore Salvataggio Ordine"}); 
    } 
});

app.post('/api/ordine/completato', async (req, res) => { 
    try { await pool.query("UPDATE ordini SET stato = 'servito' WHERE id = $1", [req.body.id]); res.json({ success: true }); } 
    catch (e) { res.status(500).json({error:"Err"}); } 
});

// API ADMIN & CONFIG (Tutti i catch ora ritornano JSON)
app.put('/api/ristorante/style/:id', async (req, res) => { try { await pool.query(`UPDATE ristoranti SET logo_url=$1, cover_url=$2, colore_sfondo=$3, colore_titolo=$4, colore_testo=$5, colore_prezzo=$6, font_style=$7 WHERE id=$8`, [req.body.logo_url, req.body.cover_url, req.body.colore_sfondo, req.body.colore_titolo, req.body.colore_testo, req.body.colore_prezzo, req.body.font_style, req.params.id]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: "Err Style" }); } });
app.get('/api/ristorante/config/:id', async (req, res) => { try { const r = await pool.query('SELECT * FROM ristoranti WHERE id = $1', [req.params.id]); res.json(r.rows[0]); } catch (e) { res.status(500).json({error:"Err Config"}); } });
app.post('/api/categorie', async (req, res) => { try { const { nome, ristorante_id, descrizione, is_bar } = req.body; const max = await pool.query('SELECT MAX(posizione) as max FROM categorie WHERE ristorante_id = $1', [ristorante_id]); const next = (max.rows[0].max || 0) + 1; const r = await pool.query('INSERT INTO categorie (nome, posizione, ristorante_id, descrizione, is_bar) VALUES ($1, $2, $3, $4, $5) RETURNING *', [nome, next, ristorante_id, descrizione||"", is_bar || false]); res.json(r.rows[0]); } catch (e) { res.status(500).json({error:"Err Cat"}); } });
app.put('/api/categorie/:id', async (req, res) => { try { await pool.query('UPDATE categorie SET nome = $1, descrizione = $2, is_bar = $3 WHERE id = $4', [req.body.nome, req.body.descrizione||"", req.body.is_bar, req.params.id]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: "Err Upd Cat" }); } });
app.delete('/api/categorie/:id', async (req, res) => { try { await pool.query('DELETE FROM categorie WHERE id = $1', [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: "Err Del Cat" }); } });
app.put('/api/categorie/riordina', async (req, res) => { const { categorie } = req.body; try { for (const cat of categorie) await pool.query('UPDATE categorie SET posizione = $1 WHERE id = $2', [cat.posizione, cat.id]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: "Err Ord Cat" }); } });
app.post('/api/prodotti', async (req, res) => { try { await pool.query('INSERT INTO prodotti (nome, prezzo, categoria, sottocategoria, descrizione, ristorante_id, immagine_url, posizione) VALUES ($1, $2, $3, $4, $5, $6, $7, 999)', [req.body.nome, req.body.prezzo, req.body.categoria, req.body.sottocategoria||"", req.body.descrizione||"", req.body.ristorante_id, req.body.immagine_url||""]); res.json({success:true}); } catch(e){ res.status(500).json({error:"Err Prod"}); } });
app.put('/api/prodotti/:id', async (req, res) => { try { await pool.query(`UPDATE prodotti SET nome=$1, prezzo=$2, categoria=$3, sottocategoria=$4, descrizione=$5, immagine_url=$6 WHERE id=$7`, [req.body.nome, req.body.prezzo, req.body.categoria, req.body.sottocategoria||"", req.body.descrizione||"", req.body.immagine_url||"", req.params.id]); res.json({ success: true }); } catch (err) { res.status(500).json({ error: "Err Upd Prod" }); } });
app.delete('/api/prodotti/:id', async (req, res) => { try { await pool.query('DELETE FROM prodotti WHERE id = $1', [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: "Err Del Prod" }); } });
app.put('/api/prodotti/riordina', async (req, res) => { const { prodotti } = req.body; try { for (const prod of prodotti) { if(prod.categoria) await pool.query('UPDATE prodotti SET posizione = $1, categoria = $2 WHERE id = $3', [prod.posizione, prod.categoria, prod.id]); else await pool.query('UPDATE prodotti SET posizione = $1 WHERE id = $2', [prod.posizione, prod.id]); } res.json({ success: true }); } catch (err) { res.status(500).json({ error: "Err Ord Prod" }); } });
app.get('/api/categorie/:ristorante_id', async (req, res) => { try { const r = await pool.query('SELECT * FROM categorie WHERE ristorante_id = $1 ORDER BY posizione ASC', [req.params.ristorante_id]); res.json(r.rows); } catch (e) { res.status(500).json({error:"Err Get Cat"}); } });
app.put('/api/ristorante/servizio/:id', async (req, res) => { try { await pool.query('UPDATE ristoranti SET servizio_attivo = $1 WHERE id = $2', [req.body.servizio_attivo, req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({error:"Err Serv"}); } });
app.post('/api/login', async (req, res) => { try { const r = await pool.query('SELECT * FROM ristoranti WHERE email = $1 AND password = $2', [req.body.email, req.body.password]); if(r.rows.length>0) res.json({success:true, user:r.rows[0]}); else res.status(401).json({success:false}); } catch(e){res.status(500).json({error:"Err Login"});} });
app.post('/api/upload', upload.single('photo'), async (req, res) => { try { res.json({ url: req.file.path }); } catch (e) { res.status(500).json({error:"Err Upload"}); } });
