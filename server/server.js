// server/server.js - VERSIONE DRAG & DROP TOTALE (CATEGORIE + PIATTI) 👑
...
// 1. MENU PUBBLICO (ORDINATO PER CATEGORIA + POSIZIONE PIATTO)
app.get('/api/menu/:slug', async (req, res) => {
    try {
        ...
        // QUERY AVANZATA: Unisce prodotti e categorie per ordinare tutto perfettamente.
        const query = `
...
// 2. RIORDINA PRODOTTI (Drag & Drop save)
app.put('/api/prodotti/riordina', async (req, res) => {
    const { prodotti } = req.body; // Array di { id, posizione }
    try {
        for (const prod of prodotti) {
            // Se c'è anche la categoria nel body (spostamento tra liste), aggiorniamo anche quella
            if(prod.categoria) {
...
// ... ALTRE ROTTE STANDARD (Config, Login, Upload, Ordini, Categorie CRUD) ...
// (Per brevità includo le essenziali per il funzionamento, ma tu mantieni le altre)

app.get('/api/ristorante/config/:id', async (req, res) => {
    try { const result = await pool.query('SELECT ordini_abilitati, servizio_attivo FROM ristoranti WHERE id = $1', [req.params.id]); res.json(result.rows[0]); } catch (e) { res.status(500).json({error:"Err"}); }
});
...
app.get('/api/categorie/:ristorante_id', async (req, res) => {
    try { const result = await pool.query('SELECT * FROM categorie WHERE ristorante_id = $1 ORDER BY posizione ASC', [req.params.ristorante_id]); res.json(result.rows); } catch (e) { res.status(500).json({error:"Err"}); }
});
app.post('/api/categorie', async (req, res) => {
    try { 
        const maxPos = await pool.query('SELECT MAX(posizione) as max FROM categorie WHERE ristorante_id = $1', [req.body.ristorante_id]);
        const nextPos = (maxPos.rows[0].max || 0) + 1;
        const result = await pool.query('INSERT INTO categorie (nome, posizione, ristorante_id) VALUES ($1, $2, $3) RETURNING *', [req.body.nome, nextPos, req.body.ristorante_id]); res.json(result.rows[0]); 
...
app.post('/api/prodotti', async (req, res) => { 
    try { 
        // Mettiamo il nuovo piatto in fondo
        const result = await pool.query('INSERT INTO prodotti (nome, prezzo, categoria, ristorante_id, immagine_url, posizione) VALUES ($1, $2, $3, $4, $5, 999) RETURNING *', [req.body.nome, req.body.prezzo, req.body.categoria, req.body.ristorante_id, req.body.immagine_url||""]); res.json({success:true}); 
    } catch(e){ res.status(500).json({error:"Err"}); } 
});
