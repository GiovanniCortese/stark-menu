...
if (!process.env.DATABASE_URL) {
    console.error("❌ ERRORE CRITICO: Manca DATABASE_URL");
    process.exit(1);
}
...
// --- SISTEMA DI AUTO-RIPARAZIONE DATABASE ---
const initDb = async () => {
    ...
    try {
        ...
        console.log("✅ Database Riparato e Pronto.");
        return true;
...
app.get('/api/cassa/storico/:ristorante_id', async (req, res) => {
    try {
        ...
        const ordini = r.rows.map(o => {
            let parsed = [];
            try { 
                parsed = typeof o.prodotti === 'string' ? JSON.parse(o.prodotti) : o.prodotti;
            } catch (e) { parsed = [{nome: "Err Dati"}]; }
