// server/server.js - VERSIONE V39 (FIX BUG CANCELLAZIONE DATI + SICUREZZA) 🛡️
...
const initDb = async () => {
    ...
    try {
        console.log("🛠️ AVVIO CONTROLLO DATABASE (V39)...");
        await client.query(`
        ...
        // CONTROLLO COLONNE AGGIUNTIVE (Come da tuo Excel)
        await client.query(`
            DO $$ BEGIN 
            ...
            END $$;
        `);
        console.log("✅ Database V39 Pronto (Safe Mode).");
        return true;
...
app.get('/api/menu/:slug', async (req, res) => {
    try {
        ...
        // LOGICA DI BLOCCO:
        // account_attivo = Abbonamento (Se false, blocco totale)
        // servizio_attivo = Cucina (Se false, ordini chiusi ma menu visibile)
        // ordini_abilitati = Legacy/Extra (Lo teniamo per sicurezza)
        
        // Se account_attivo esiste nel DB usalo, altrimenti fallback su true
        ...
        res.json({ 
            ...
            kitchen_active: data.servizio_attivo, // Usiamo servizio_attivo come stato cucina
            
            // Campi raw per compatibilità
            account_attivo: data.account_attivo,
...
app.put('/api/ristorante/servizio/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // Il ristoratore tocca 'servizio_attivo' o 'ordini_abilitati' a seconda del frontend.
        // Gestiamo entrambi per sicurezza.
        
        const check = await pool.query('SELECT account_attivo FROM ristoranti WHERE id = $1', [id]);
        if (check.rows.length === 0) return res.status(404).json({ error: "Ristorante non trovato" });

        // BLOCCO ABBONAMENTO
        if (check.rows[0].account_attivo === false) {
            return res.status(403).json({ error: "ABBONAMENTO SOSPESO. Impossibile modificare." });
        }

        // Aggiorna servizio_attivo (Cucina)
...
app.post('/api/super/ristoranti', async (req, res) => { 
    try { 
        await pool.query(`INSERT INTO ristoranti (nome, slug, email, telefono, password, account_attivo, servizio_attivo, ordini_abilitati) VALUES ($1, $2, $3, $4, $5, TRUE, TRUE, TRUE)`, [req.body.nome, req.body.slug, req.body.email, req.body.telefono, req.body.password || 'tonystark']); 
        res.json({ success: true }); 
    } catch (e) { res.status(500).json({error: "Err"}); } 
});

// >>>>>> QUI C'ERA IL BUG! ORA È RISOLTO <<<<<<
app.put('/api/super/ristoranti/:id', async (req, res) => { 
    try { 
        const { id } = req.params;

        // 1. GESTIONE TOGGLE (Aggiorna SOLO il campo richiesto e si ferma)
        ...
        // Caso: Cambio Servizio/Cucina (servizio_attivo)
        if (req.body.servizio_attivo !== undefined) { 
            await pool.query('UPDATE ristoranti SET servizio_attivo = $1 WHERE id = $2', [req.body.servizio_attivo, id]); 
            return res.json({ success: true }); 
        }

        // Caso: Legacy (ordini_abilitati)
        if (req.body.ordini_abilitati !== undefined) {
             await pool.query('UPDATE ristoranti SET ordini_abilitati = $1 WHERE id = $2', [req.body.ordini_abilitati, id]);
             return res.json({ success: true });
        }

        // 2. GESTIONE MODIFICA ANAGRAFICA (Nome, Slug, ecc.)
        // ⚠️ PROTEZIONE: Se non c'è il nome o lo slug, NON fare update generico!
        if (!req.body.nome || !req.body.slug) {
            // Se arriviamo qui senza nome/slug e senza toggle, ignoriamo per sicurezza invece di cancellare tutto.
            console.warn("⚠️ Tentativo di update anagrafica senza dati per ID:", id);
...
initDb().then((ready) => { if (ready) app.listen(port, () => console.log(`🚀 SERVER V39 (SAFE MODE) - Porta ${port}`)); });