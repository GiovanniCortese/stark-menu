// server/server.js - VERSIONE V34 (CORREZIONE RIORDINO + STABILITÀ) 🍕
...
app.get('/api/menu/:slug', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ error: "Errore del server" }); }
});
...
// 3. AGGIORNAMENTO ELEMENTI ORDINE
app.put('/api/ordine/:id/update-items', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Errore Aggiornamento Elementi" }); 
    }
});

// 4. PAGAMENTO (UNIONE TOTALE)
app.post('/api/cassa/paga-tavolo', async (req, res) => {
    ...
    try {
        ...
        await client.query('ROLLBACK'); 
        console.error("Errore Pagamento Unione:", e);
