// server/server.js - VERSIONE V35 (NUOVO COMANDO ACCOUNT_ATTIVO) 💀
...
const initDb = async () => {
    ...
    try {
        console.log("🛠️ AVVIO CONTROLLO DATABASE (V35 - NUOVO CAMPO ACCOUNT)...");
        await client.query(`
        ...
        // MODIFICA V35: AGGIUNTA COLONNA "account_attivo" (IL NUOVO COMANDO)
        await client.query(`
            DO $$ BEGIN 
            ...
            END $$;
        `);
        console.log("✅ Database V35 Pronto (account_attivo creato).");
        return true;
...
app.get('/api/menu/:slug', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            // QUI PASSIAMO IL NUOVO PARAMETRO
            account_attivo: data.account_attivo !== false, // Default true
            servizio_attivo: data.servizio_attivo, 
            ordini_abilitati: data.ordini_abilitati, // Legacy check
            menu: menu.rows 
        });
...
app.get('/api/polling/:ristorante_id', async (req, res) => { 
    try { 
        ...
        const ordini = r.rows.map(o => {
            ...
            parsed = parsed.map((item, idx) => ({ ...item, uniqId: item.uniqId || `${o.id}_${idx}_${Date.now()}`, stato: item.stato || 'in_attesa' }));
...
app.put('/api/ordine/:id/update-items', async (req, res) => {
    try {
        ...
        let paramIndex = 2;
        if (totale !== undefined) { query += `, totale = $${paramIndex}`; params.push(totale); paramIndex++; }
        if (logMsg) { const timestamp = new Date().toLocaleString('it-IT', { timeZone: 'Europe/Rome' }); const logEntry = `\n[${timestamp}] ${logMsg}`; query += `, dettagli = COALESCE(dettagli, '') || $${paramIndex}`; params.push(logEntry); paramIndex++; }
        query += ` WHERE id = $${paramIndex}`; params.push(id);
        await pool.query(query, params);
        res.json({ success: true });
    } catch (e) { console.error("Errore Update:", e); res.status(500).json({ error: "Errore Update Items" }); }
});
...
app.post('/api/cassa/paga-tavolo', async (req, res) => {
    ...
    try {
        ...
        const result = await client.query("SELECT * FROM ordini WHERE ristorante_id = $1 AND tavolo = $2 AND stato != 'pagato' ORDER BY data_ora ASC", [ristorante_id, String(tavolo)]);
        if (result.rows.length === 0) { await client.query('ROLLBACK'); return res.json({ success: true }); }
        let primoOrdine = result.rows[0]; let prodottiUnificati = []; let totaleUnificato = 0; let logUnificato = "";
        result.rows.forEach(ord => {
            let prodottiProd = []; try { prodottiProd = typeof ord.prodotti === 'string' ? JSON.parse(ord.prodotti || "[]") : ord.prodotti; } catch(e) {}
            if(Array.isArray(prodottiProd)) prodottiUnificati = [...prodottiUnificati, ...prodottiProd];
            totaleUnificato += Number(ord.totale || 0);
            if (ord.dettagli && ord.dettagli.trim() !== "") { logUnificato += `\n--- ORDINE PARZIALE #${ord.id} ---\n` + ord.dettagli.trim() + "\n"; }
        });
        const timeLog = new Date().toLocaleString('it-IT', { timeZone: 'Europe/Rome' });
        logUnificato += `\n================================\n[${timeLog}] 💰 TAVOLO CHIUSO. TOTALE: ${totaleUnificato.toFixed(2)}€`;
        await client.query("UPDATE ordini SET stato = 'pagato', prodotti = $1, totale = $2, dettagli = $3 WHERE id = $4", [JSON.stringify(prodottiUnificati), totaleUnificato, logUnificato, primoOrdine.id]);
        const idsDaCancellare = result.rows.slice(1).map(o => o.id);
        if (idsDaCancellare.length > 0) { await client.query("DELETE FROM ordini WHERE id = ANY($1::int[])", [idsDaCancellare]); }
        await client.query('COMMIT'); res.json({ success: true });
    } catch (e) { await client.query('ROLLBACK'); console.error("Errore Pagamento Merge:", e); res.status(500).json({ error: "Errore Pagamento" }); } finally { client.release(); }
});
...
app.post('/api/ordine', async (req, res) => { 
    try { 
        ...
        await pool.query("INSERT INTO ordini (ristorante_id, tavolo, prodotti, totale, stato, dettagli) VALUES ($1, $2, $3, $4, 'in_attesa', $5)", [ristorante_id, String(tavolo), prodottiStr, totale, logMsg]); 
        res.json({ success: true }); 
...
app.put('/api/categorie/riordina', async (req, res) => {
    const { categorie } = req.body; const client = await pool.connect();
    try { await client.query('BEGIN'); for (const cat of categorie) { await client.query('UPDATE categorie SET posizione = $1 WHERE id = $2', [cat.posizione, cat.id]); } await client.query('COMMIT'); res.json({ success: true }); } 
...
app.put('/api/prodotti/riordina', async (req, res) => {
    const { prodotti } = req.body; const client = await pool.connect();
    try { await client.query('BEGIN'); for (const prod of prodotti) { if (prod.categoria) { await client.query('UPDATE prodotti SET posizione = $1, categoria = $2 WHERE id = $3', [prod.posizione, prod.categoria, prod.id]); } else { await client.query('UPDATE prodotti SET posizione = $1 WHERE id = $2', [prod.posizione, prod.id]); } } await client.query('COMMIT'); res.json({ success: true }); } 
    catch (err) { await client.query('ROLLBACK'); res.status(500).json({ error: "Err" }); } finally { client.release(); }
});

// --- API SUPER ADMIN (MODIFICATE PER account_attivo) ---
app.get('/api/super/ristoranti', async (req, res) => { 
    try { 
        // V35: Selezioniamo anche account_attivo
        const r = await pool.query('SELECT id, nome, slug, ordini_abilitati, servizio_attivo, account_attivo, email, telefono FROM ristoranti ORDER BY id ASC'); 
        res.json(r.rows); 
    } catch (e) { res.status(500).json({error:"Err"}); } 
});

app.post('/api/super/ristoranti', async (req, res) => { try { await pool.query(`INSERT INTO ristoranti (nome, slug, email, telefono, password, account_attivo, servizio_attivo) VALUES ($1, $2, $3, $4, $5, TRUE, FALSE)`, [req.body.nome, req.body.slug, req.body.email, req.body.telefono, req.body.password || 'tonystark']); res.json({ success: true }); } catch (e) { res.status(500).json({error: "Err"}); } });

app.put('/api/super/ristoranti/:id', async (req, res) => { 
    try { 
        // V35: GESTIONE AGGIORNAMENTO account_attivo
        if (req.body.account_attivo !== undefined) { 
            await pool.query('UPDATE ristoranti SET account_attivo = $1 WHERE id = $2', [req.body.account_attivo, req.params.id]); 
            return res.json({ success: true }); 
        }
        
        // Gestione aggiornamento solo servizio_attivo (cucina)
        ...
        if (req.body.password) { sql += ", password=$5 WHERE id=$6"; params.push(req.body.password, req.params.id); } else { sql += " WHERE id=$5"; params.push(req.params.id); } 
...
// V35: LOGIN: Se account_attivo è FALSE, NON ENTRA.
app.post('/api/login', async (req, res) => { 
    try { 
        const r = await pool.query('SELECT * FROM ristoranti WHERE email = $1 AND password = $2', [req.body.email, req.body.password]); 
        if(r.rows.length>0) {
            if (r.rows[0].account_attivo === false) return res.status(403).json({success:false, error: "ACCOUNT SOSPESO"});
...
// V35: MODIFICA SERVIZIO (CUCINA). CONTROLLA SE account_attivo è TRUE
app.put('/api/ristorante/servizio/:id', async (req, res) => { 
    try { 
        // 1. Check account_attivo
        const check = await pool.query('SELECT account_attivo FROM ristoranti WHERE id = $1', [req.params.id]);
        if(check.rows.length > 0 && check.rows[0].account_attivo === false) {
             return res.status(403).json({ error: "ATTIVITÀ SOSPESA" });
        }
        await pool.query('UPDATE ristoranti SET servizio_attivo = $1 WHERE id = $2', [req.body.servizio_attivo, req.params.id]); 
        res.json({ success: true }); 
    } catch (e) { res.status(500).json({error:"Err"}); } 
...
initDb().then((ready) => { if (ready) app.listen(port, () => console.log(`🚀 SERVER V35 (NUOVO COMANDO) - Porta ${port}`)); });