// server/server.js - VERSIONE V15 (SAFE BOOT: AVVIO SINCRONIZZATO) 🛡️
...
// Verifica presenza DATABASE_URL per evitare crash silenziosi
...
const initDb = async () => {
    ...
    try {
        console.log("🛠️ Connessione al DB riuscita. Verifica tabelle...");
        ...
        // Auto-Fix Colonne Ordini (Fondamentale per sbloccare il tasto Invia)
        ...
        console.log("✅ Database pronto. Avvio server...");
        return true;
    } catch (err) {
        console.error("❌ Errore durante initDb:", err);
        return false;
    } finally {
        client.release();
    }
...
app.get('/api/menu/:slug', async (req, res) => {
    try {
        ...
        const rist = await pool.query(`
            ...
            FROM ristoranti WHERE slug = $1`, [slug]);
        ...
        res.json({ 
            ...
            style: {
                ...
                title: data.colore_titolo, text: data.colore_testo, price: data.colore_prezzo, font: data.font_style
            },
...
app.post('/api/import-excel', uploadFile.single('file'), async (req, res) => {
    ...
    try {
        ...
        for (const row of data) {
            ...
            await pool.query(
                `INSERT INTO prodotti (nome, prezzo, categoria, sottocategoria, descrizione, ristorante_id, posizione) VALUES ($1, $2, $3, $4, $5, $6, 999)`,
...
app.put('/api/prodotti/:id', async (req, res) => {
    try {
        await pool.query(`UPDATE prodotti SET nome=$1, prezzo=$2, categoria=$3, sottocategoria=$4, descrizione=$5, immagine_url=$6 WHERE id=$7`, [req.body.nome, req.body.prezzo, req.body.categoria, req.body.sottocategoria||"", req.body.descrizione||"", req.body.immagine_url||"", req.params.id]);
        res.json({ success: true });
...
app.put('/api/prodotti/riordina', async (req, res) => {
    ...
    try {
        for (const prod of prodotti) {
            if(prod.categoria) await pool.query('UPDATE prodotti SET posizione = $1, categoria = $2 WHERE id = $3', [prod.posizione, prod.categoria, prod.id]);
            else await pool.query('UPDATE prodotti SET posizione = $1 WHERE id = $2', [prod.posizione, prod.id]);
        }
        res.json({ success: true });
...
app.put('/api/categorie/riordina', async (req, res) => {
    ...
    try {
        for (const cat of categorie) await pool.query('UPDATE categorie SET posizione = $1 WHERE id = $2', [cat.posizione, cat.id]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: "Err" }); }
});
...
app.put('/api/ristorante/servizio/:id', async (req, res) => { 
    try { await pool.query('UPDATE ristoranti SET servizio_attivo = $1 WHERE id = $2', [req.body.servizio_attivo, req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({error:"Err"}); } 
});

// 11. GET CATEGORIE
app.get('/api/categorie/:ristorante_id', async (req, res) => { 
    try { const r = await pool.query('SELECT * FROM categorie WHERE ristorante_id = $1 ORDER BY posizione ASC', [req.params.ristorante_id]); res.json(r.rows); } catch (e) { res.status(500).json({error:"Err"}); } 
});

// 12. CREA CATEGORIA
app.post('/api/categorie', async (req, res) => { 
    try { const max = await pool.query('SELECT MAX(posizione) as max FROM categorie WHERE ristorante_id = $1', [req.body.ristorante_id]); const next = (max.rows[0].max || 0) + 1; const r = await pool.query('INSERT INTO categorie (nome, posizione, ristorante_id, descrizione) VALUES ($1, $2, $3, $4) RETURNING *', [req.body.nome, next, req.body.ristorante_id, req.body.descrizione||""]); res.json(r.rows[0]); } catch (e) { res.status(500).json({error:"Err"}); } 
});

// 13. ELIMINA CATEGORIA
app.delete('/api/categorie/:id', async (req, res) => { 
    try { await pool.query('DELETE FROM categorie WHERE id = $1', [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: "Err" }); } 
});

// 14. LOGIN
app.post('/api/login', async (req, res) => { 
    try { const r = await pool.query('SELECT * FROM ristoranti WHERE email = $1 AND password = $2', [req.body.email, req.body.password]); if(r.rows.length>0) res.json({success:true, user:r.rows[0]}); else res.status(401).json({success:false}); } catch(e){res.status(500).json({error:"Err"});} 
});

// 15. UPLOAD FOTO
app.post('/api/upload', upload.single('photo'), async (req, res) => { 
    try { res.json({ url: req.file.path }); } catch (e) { res.status(500).json({error:"Err"}); } 
});
...
app.get('/api/polling/:ristorante_id', async (req, res) => { 
    try { 
        ...
        const ordini = r.rows.map(o => ({...o, prodotti: JSON.parse(o.prodotti || o.dettagli || "[]")}));
...
app.post('/api/ordine/completato', async (req, res) => { 
    try { await pool.query("UPDATE ordini SET stato = 'completato' WHERE id = $1", [req.body.id]); res.json({ success: true }); } catch (e) { res.status(500).json({error:"Err"}); } 
});

// 19. ELIMINA PRODOTTO
app.delete('/api/prodotti/:id', async (req, res) => { 
    try { await pool.query('DELETE FROM prodotti WHERE id = $1', [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: "Err" }); } 
});
...
// Questo è il pezzo fondamentale che mancava nella V14 per evitare il blocco del deploy
initDb().then((ready) => {
    if (ready) {
        app.listen(port, () => console.log(`🚀 SERVER AVVIATO SU PORTA ${port}`));
    } else {
        console.log("❌ Impossibile avviare il server: DB Error");
    }
});