// client/src/Haccp.jsx - V13 FIX MOBILE & ALLINEAMENTO TEMPERATURE
...
function Haccp() {
  ...
  return (
    <div className="haccp-container" style={{minHeight:'100vh', background:'#ecf0f1', padding:'10px', fontFamily:'sans-serif', paddingBottom:'80px'}}>
      
      {!scanId && (
          <div className="no-print" style={{display:'flex', flexDirection:'column', gap:15, marginBottom:20}}>
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                   <h1 style={{margin:0, color:'#2c3e50', fontSize:'24px'}}>🛡️ HACCP</h1>
                   <button onClick={()=>{localStorage.removeItem(`haccp_session_${slug}`); setIsAuthorized(false)}} style={{background:'#e74c3c', color:'white', border:'none', padding:'8px 15px', borderRadius:5}}>ESCI</button>
              </div>

              {/* NAVIGAZIONE SCROLLABILE SU MOBILE */}
              <div style={{display:'flex', gap:10, overflowX:'auto', paddingBottom:5}}>
                  {['temperature', 'merci', 'calendario', 'etichette', 'setup'].map(t => (
                      <button key={t} onClick={()=>setTab(t)} style={{whiteSpace:'nowrap', padding:'10px 15px', borderRadius:5, border:'none', cursor:'pointer', fontWeight:'bold', textTransform:'uppercase', background: tab===t ? '#2c3e50' : 'white', color: tab===t ? 'white' : '#333'}}>
                        {t==='merci' ? '📦 Merci' : (t==='setup' ? '⚙️ Setup' : t)}
                      </button>
                  ))}
              </div>
              
              <div style={{display:'flex', gap:5, overflowX:'auto'}}>
                  <button onClick={()=>openDownloadModal('temperature')} style={{background:'#27ae60', color:'white', border:'none', padding:'8px', borderRadius:3, fontSize:12, whiteSpace:'nowrap'}}>⬇ Temp</button>
                  <button onClick={()=>openDownloadModal('merci')} style={{background:'#f39c12', color:'white', border:'none', padding:'8px', borderRadius:3, fontSize:12, whiteSpace:'nowrap'}}>⬇ Merci</button>
                  <button onClick={()=>openDownloadModal('assets')} style={{background:'#34495e', color:'white', border:'none', padding:'8px', borderRadius:3, fontSize:12, whiteSpace:'nowrap'}}>⬇ Macchine</button>
              </div>
          </div>
      )}
      ...
      {/* 1. TEMPERATURE */}
      {tab === 'temperature' && (
          <div className="no-print" style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:15}}>
              {assetsToDisplay.map(asset => {
                  ...
                  if(asset.stato === 'spento') {
                      return (
                          <div key={asset.id} style={{background:'#e0e0e0', padding:15, borderRadius:10, border:'2px solid #999', opacity:0.7, position:'relative'}}>
                              ...
                              <div style={{height:40, background:'#ccc', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', color:'#777', fontWeight:'bold', fontSize:12, marginTop:10}}>
                                  SPENTO
                              </div>
                          </div>
                      );
                  ...
                  if (todayLog && !isInputActive) {
                      ...
                      return (
                          <div key={asset.id} style={{background:'#eafaf1', padding:15, borderRadius:10, border:'2px solid #27ae60'}}>
                              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                  <h3 style={{margin:0, color:'#27ae60'}}>✅ {asset.nome}</h3>
                                  <span style={{fontSize:'22px', fontWeight:'bold'}}>{todayLog.valore === 'OFF' ? 'SPENTO' : todayLog.valore + '°C'}</span>
                              </div>
                              <div style={{fontSize:'12px', color:'#555', marginTop:5}}>
                                  {todayLog.conformita ? `Ore ${timeStr}` : `⚠️ ${todayLog.azione_correttiva}`}
                              </div>
                              <button onClick={() => abilitaNuovaMisurazione(asset)} style={{marginTop:15, width:'100%', background:'#f39c12', color:'white', border:'none', padding:10, borderRadius:5, fontWeight:'bold'}}>✏️ MODIFICA</button>
                          </div>
                      );
                  }
                  
                  // --- FIX GRAFICO INPUT TEMPERATURE (FLEXBOX) ---
                  return (
                      <div key={asset.id} style={{background:'white', padding:15, borderRadius:10, boxShadow:'0 2px 5px rgba(0,0,0,0.1)', borderTop:'5px solid #bdc3c7'}}>
                           ...
                           <div style={{display:'flex', fontSize:'11px', fontWeight:'bold', color:'#7f8c8d', marginBottom:5}}>
                                ...
                                <div style={{width:130, textAlign:'center'}}>AZIONI</div>
                           </div>

                           {/* RIGA FLEXBOX PERFETTAMENTE ALLINEATA */}
                           <div style={{display:'flex', gap:8, height:45, alignItems:'stretch'}}>
                              {/* INPUT (Flessibile, si stringe se necessario) */}
                              <input type="number" step="0.1" placeholder="°C" 
                                   ...
                                   style={{flex:1, width:'0px', minWidth:0, borderRadius:5, border:'1px solid #ddd', fontSize:20, textAlign:'center', fontWeight:'bold'}} 
                              />
                              
                              {/* PULSANTI (Larghezza fissa, non si schiacciano) */}
                              <div style={{display:'flex', gap:5, flexShrink:0}}>
                                  <button onClick={()=>registraTemperatura(asset, true)} 
                                          style={{width:45, background:'#95a5a6', color:'white', border:'none', borderRadius:5, fontWeight:'bold', fontSize:'10px', display:'flex', alignItems:'center', justifyContent:'center'}}>
                                      OFF
                                  </button>

                                  <label style={{width:45, background: currentData.photo ? '#2ecc71' : '#f1f2f6', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', border:'1px solid #ddd', cursor:'pointer'}}>
                                  ...
                                  <button onClick={()=>registraTemperatura(asset, false)} 
                                          style={{width:60, background:'#2c3e50', color:'white', border:'none', borderRadius:5, fontWeight:'bold', fontSize:'12px'}}>
                                      SALVA
                                  </button>
                              </div>
                           </div>

                           {isInputActive && getTodayLog(asset.id) && <button onClick={()=>{const n={...tempInput}; delete n[asset.id]; setTempInput(n);}} style={{marginTop:10, width:'100%', fontSize:12, background:'transparent', border:'none', color:'#999'}}>Annulla Modifica</button>}
                      </div>
                  );
              })}
          </div>
      ...
      {/* 2. MERCI (CON SCROLL MOBILE) */}
      {tab === 'merci' && !scanId && (
          <div className="no-print">
              <div style={{background:'white', padding:15, borderRadius:10, marginBottom:20, borderLeft: merciForm.id ? '5px solid #f39c12' : '5px solid #27ae60'}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10}}>
                      <h3 style={{margin:0}}>{merciForm.id ? '✏️ Modifica' : '📥 Nuovo Arrivo'}</h3>
                      {merciForm.id && <button onClick={resetMerciForm} style={{background:'#e74c3c', color:'white', border:'none', borderRadius:5, padding:'5px 10px'}}>X</button>}
                  </div>
                  <form onSubmit={salvaMerci} style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px, 1fr))', gap:10}}>
                      <div><label style={{fontSize:10}}>Data</label><input type="date" value={merciForm.data_ricezione} onChange={e=>setMerciForm({...merciForm, data_ricezione:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} required /></div>
                      <div><label style={{fontSize:10}}>Fornitore</label><input value={merciForm.fornitore} onChange={e=>setMerciForm({...merciForm, fornitore:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} required /></div>
                      <div><label style={{fontSize:10}}>Prodotto</label><input value={merciForm.prodotto} onChange={e=>setMerciForm({...merciForm, prodotto:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} required /></div>
                      <div><label style={{fontSize:10}}>Quantità</label><input value={merciForm.quantita} onChange={e=>setMerciForm({...merciForm, quantita:e.target.value})} placeholder="Es. 10kg" style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} /></div>
                      <div><label style={{fontSize:10}}>Lotto</label><input value={merciForm.lotto} onChange={e=>setMerciForm({...merciForm, lotto:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} /></div>
                      <div><label style={{fontSize:10}}>Scadenza</label><input type="date" value={merciForm.scadenza} onChange={e=>setMerciForm({...merciForm, scadenza:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} /></div>
                      <div><label style={{fontSize:10}}>Temp °C</label><input type="number" step="0.1" value={merciForm.temperatura} onChange={e=>setMerciForm({...merciForm, temperatura:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} /></div>
                      
                      <div style={{gridColumn:'1/-1'}}><label style={{fontSize:10}}>Note</label><input value={merciForm.note} onChange={e=>setMerciForm({...merciForm, note:e.target.value})} style={{width:'100%', padding:8, border:'1px solid #ddd', boxSizing:'border-box'}} /></div>
                      
                      <div style={{gridColumn:'1/-1', display:'flex', gap:10, alignItems:'center', flexWrap:'wrap'}}>
                          <label style={{fontSize:12}}><input type="checkbox" checked={merciForm.conforme} onChange={e=>setMerciForm({...merciForm, conforme:e.target.checked})} /> Conforme</label>
                          <label style={{fontSize:12}}><input type="checkbox" checked={merciForm.integro} onChange={e=>setMerciForm({...merciForm, integro:e.target.checked})} /> Integro</label>
                          <label style={{background: merciForm.allegato_url ? '#2ecc71' : '#ecf0f1', padding:'8px', borderRadius:5, fontSize:12, cursor:'pointer'}}>
                      ...
                      <button style={{gridColumn:'1/-1', background: merciForm.id ? '#f39c12' : '#27ae60', color:'white', border:'none', padding:'12px', borderRadius:5, fontWeight:'bold', marginTop:5}}>
                          ...
                          {merci.map(m => (
                              <tr key={m.id} style={{borderBottom:'1px solid #eee'}}>
                                  ...
                                  <td style={{padding:8, display:'flex', gap:5}}>
                                      {m.allegato_url && <a href={m.allegato_url} target="_blank" style={{background:'#3498db', color:'white', padding:'3px 6px', borderRadius:3, textDecoration:'none'}}>📎</a>}
                                      <button onClick={()=>iniziaModificaMerci(m)} style={{background:'#f39c12', color:'white', border:'none', borderRadius:3, padding:'3px 6px'}}>✏️</button>
                                      <button onClick={()=>eliminaMerce(m.id)} style={{background:'#e74c3c', color:'white', border:'none', borderRadius:3, padding:'3px 6px'}}>🗑️</button>
                                  </td>
                              </tr>
                          ))}
                      </tbody>
          ...
          <div className="no-print" style={{display:'flex', flexDirection:'column', gap:20}}>
             <div style={{background:'white', padding:20, borderRadius:10}}>
                 ...
                 <form onSubmit={handlePrintLabel} style={{display:'flex', flexDirection:'column', gap:10}}>
                    <input placeholder="Prodotto" required value={labelData.prodotto} onChange={e=>setLabelData({...labelData, prodotto:e.target.value})} style={{padding:12, border:'1px solid #ccc', borderRadius:5}} />
                    <select value={labelData.tipo} onChange={handleLabelTypeChange} style={{padding:12, border:'1px solid #ccc', borderRadius:5}}>
                    ...
                    <input type="number" value={labelData.giorni_scadenza} onChange={e=>setLabelData({...labelData, giorni_scadenza:e.target.value})} style={{padding:12, border:'1px solid #ccc', borderRadius:5}} />
                    <input placeholder="Operatore" value={labelData.operatore} onChange={e=>setLabelData({...labelData, operatore:e.target.value})} style={{padding:12, border:'1px solid #ccc', borderRadius:5}} />
                    <button style={{background:'#2980b9', color:'white', border:'none', padding:15, marginTop:10, borderRadius:5, fontWeight:'bold'}}>STAMPA</button>
                 </form>
             </div>
          </div>
      )}
      ...
      {tab === 'setup' && !scanId && (
          <div className="no-print">
              <div style={{display:'flex', justifyContent:'space-between', marginBottom:20, alignItems:'center'}}>
                  <h2>Macchinari</h2>
                  <button onClick={()=>apriModaleAsset()} style={{background:'#27ae60', color:'white', border:'none', padding:'10px', borderRadius:5, fontWeight:'bold'}}>+ Nuovo</button>
              </div>
                  ...
                  {assets.map(a => (
                      <div key={a.id} style={{background: 'white', padding:15, borderRadius:10, borderLeft: '4px solid #34495e', boxShadow:'0 2px 5px rgba(0,0,0,0.1)'}}>
                          ...
                          <div style={{marginTop:10, display:'flex', gap:5}}>
                              <button onClick={()=>setShowQRModal(a)} style={{background:'#34495e', color:'white', border:'none', padding:'8px', borderRadius:3, flex:1, fontWeight:'bold', fontSize:12}}>QR</button>
                              <button onClick={()=>apriModaleAsset(a)} style={{background:'#f39c12', color:'white', border:'none', padding:'8px', borderRadius:3, flex:1, fontWeight:'bold', fontSize:12}}>Modifica</button>
                          </div>
                          
                          <div style={{marginTop:10, display:'flex', gap:10, fontSize:12}}>
                                <button onClick={() => a.foto_url && setPreviewImage(a.foto_url)} style={{background:'transparent', border:'none', color: a.foto_url ? '#3498db' : '#ccc', padding:0}}>📸 Foto</button>
                                <button onClick={() => a.etichetta_url && setPreviewImage(a.etichetta_url)} style={{background:'transparent', border:'none', color: a.etichetta_url ? '#e67e22' : '#ccc', padding:0}}>📄 Etichetta</button>
                          </div>
                      </div>
                  ))}
              </div>
      ...
      {/* --- MODALI (DOWNLOAD, ASSET, QR, PREVIEW) --- */}
      {showDownloadModal && (
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:2000, padding:20}}>
              <div style={{background:'white', padding:25, borderRadius:10, textAlign:'center', width:'100%', maxWidth:'350px'}}>
                  <h3 style={{marginTop:0}}>Scarica Report</h3>
                  <div style={{marginBottom:15, background:'#f9f9f9', padding:10, borderRadius:5, display:'flex', justifyContent:'center', gap:10}}>
                       <button onClick={()=>setDownloadFormat('excel')} style={{background: downloadFormat==='excel'?'#27ae60':'#eee', color:downloadFormat==='excel'?'white':'black', padding:'5px 15px', border:'none', borderRadius:20}}>Excel</button>
                       <button onClick={()=>setDownloadFormat('pdf')} style={{background: downloadFormat==='pdf'?'#e74c3c':'#eee', color:downloadFormat==='pdf'?'white':'black', padding:'5px 15px', border:'none', borderRadius:20}}>PDF</button>
                  </div>
                  <div style={{display:'flex', flexDirection:'column', gap:8}}>
                      <div style={{display:'flex', gap:5}}>
                          ...
                          <button onClick={()=>executeDownload('custom-month')} style={{padding:'0 15px', background:'#8e44ad', color:'white', border:'none', borderRadius:5}}>OK</button>
                      </div>
                      <button onClick={()=>executeDownload('month')} style={{padding:12, background:'#2980b9', color:'white', border:'none', borderRadius:5}}>Ultimi 30 Giorni</button>
                      <button onClick={()=>executeDownload('all')} style={{padding:12, background:'#2c3e50', color:'white', border:'none', borderRadius:5}}>Storico Completo</button>
                  </div>
                  <button onClick={()=>setShowDownloadModal(false)} style={{marginTop:20, background:'transparent', border:'none', color:'#999', textDecoration:'underline'}}>Chiudi</button>
              </div>
          </div>
      )}
      ...
      {showAssetModal && (
         <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20}}>
            <div style={{background:'white', padding:20, width:'100%', maxWidth:'400px', borderRadius:10, maxHeight:'85vh', overflowY:'auto'}}>
               <h3 style={{marginTop:0}}>{editingAsset ? 'Modifica' : 'Nuovo'}</h3>
               <form onSubmit={salvaAsset} style={{display:'flex', flexDirection:'column', gap:10}}>
                   <input value={assetForm.nome} onChange={e=>setAssetForm({...assetForm, nome:e.target.value})} placeholder="Nome" style={{padding:10, border:'1px solid #ccc', borderRadius:5}} required />
                   ...
                   <select value={assetForm.stato} onChange={e=>setAssetForm({...assetForm, stato:e.target.value})} style={{padding:10, borderRadius:5, border:'1px solid #ccc'}}>
                       <option value="attivo">✅ Attiva</option><option value="spento">⛔ Spenta</option>
                   </select>
                   <select value={assetForm.tipo} onChange={e=>setAssetForm({...assetForm, tipo:e.target.value})} style={{padding:10, borderRadius:5, border:'1px solid #ccc'}}>
                       <option value="frigo">Frigo</option><option value="cella">Cella</option><option value="vetrina">Vetrina</option><option value="congelatore">Congelatore</option>
                   </select>
                   <button style={{background:'#27ae60', color:'white', border:'none', padding:12, borderRadius:5, marginTop:10}}>SALVA</button>
                   <button type="button" onClick={()=>setShowAssetModal(false)} style={{background:'#95a5a6', color:'white', border:'none', padding:12, borderRadius:5}}>Annulla</button>
               </form>
            </div>
         </div>
      )}
      ...
      {previewImage && (
          <div onClick={() => setPreviewImage(null)} style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.9)', zIndex:3000, display:'flex', alignItems:'center', justifyContent:'center'}}>
              <img src={previewImage} alt="Preview" style={{maxWidth:'95%', maxHeight:'90%', borderRadius:5}} />
          </div>
      )}
      ...
      {showQRModal && (
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1500}}>
              <div style={{background:'white', padding:30, textAlign:'center', borderRadius:10}}>
                  ...
                  <br/><br/><button onClick={executePrintQR}>STAMPA</button><button onClick={()=>setShowQRModal(null)} style={{marginLeft:10}}>CHIUDI</button>
              </div>
          </div>
      )}
      ...
      <style>{`@media print { .no-print { display: none !important; } .print-area { z-index: 9999; display: flex !important; } body { margin: 0; padding: 0; } }`}</style>
    </div>
  );
}
