  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", borderBottom: `1px solid ${style?.text || "#ccc"}`, paddingBottom: "10px" }}>
        ...
        <button onClick={onClose} style={{ background: "transparent", border: "none", color: titleColor, fontSize: "24px", cursor: "pointer" }}>
          ✕
        </button>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", maxWidth: "600px", margin: "0 auto", width: "100%" }}>
        {carrello.length === 0 && (
          <p className="notranslate" style={{ color: style?.text || "#fff", textAlign: "center" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.1)", padding: "15px", borderRadius: "10px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <button
                ...
                style={{ width: 30, height: 30, borderRadius: "50%", border: "none", fontSize: "18px", cursor: "pointer", fontWeight: "bold" }}
              >
                -
              </button>
        ...
        {(() => {
          ...
          return coursePresenti.map((courseNum, index) => (
            <div key={courseNum} style={{ marginBottom: "25px" }}>
              <h3 className="notranslate" style={{ margin: "0 0 10px 0", color: coloriPortata[index] || "#ccc", borderBottom: `2px solid ${coloriPortata[index] || "#ccc"}`, display: "inline-block", paddingRight: 20 }}>
                    ...
                    <div key={item.tempId} style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, padding: 15, marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: "bold", fontSize: "1.1rem", color: titleColor }}>
                          ...
                          {qtaLabel && <span style={{ color: priceColor, fontSize: "0.9rem" }}>({qtaLabel})</span>}
                        </div>
                          ...
                          <div className="notranslate" style={{ marginTop: "6px" }}>
                            {allergeniItem.filter((a) => !a.includes("❄️")).length > 0 && (
                              <div style={{ fontSize: "10px", color: "#ff7675", fontWeight: "bold", textTransform: "uppercase" }}>
                                ⚠️ {t?.allergens || "Allergeni"}: {allergeniItem.filter((a) => !a.includes("❄️")).join(", ")}
                              </div>
                            )}
                        ...
                        <div className="notranslate" style={{ color: priceColor, fontSize: "0.9rem", marginTop: "8px", fontWeight: "bold" }}>
                      ...
                      <div className="notranslate" style={{ display: "flex", flexDirection: "column", gap: 5, marginLeft: "10px" }}>
                        <div style={{ display: "flex", gap: 5, marginBottom: 5 }}>
                          <button onClick={() => onChangeCourse(item.tempId, -1)} style={{ background: "#ecf0f1", color: "#333", border: "none", fontSize: "0.8rem", padding: "5px 8px", borderRadius: "4px", cursor: "pointer" }}>
                            ⬆️
                          </button>
                          <button onClick={() => onChangeCourse(item.tempId, 1)} style={{ background: "#ecf0f1", color: "#333", border: "none", fontSize: "0.8rem", padding: "5px 8px", borderRadius: "4px", cursor: "pointer" }}>
                            ⬇️
                        ...
                        <button onClick={() => onRemoveItem(item.tempId)} style={{ background: "#e74c3c", color: "white", fontSize: "0.8rem", padding: "5px 10px", borderRadius: "4px", border: "none", cursor: "pointer", fontWeight: "bold" }}>
                          X
                        </button>
                      </div>
                    </div>
            ...
            {carrello
              ...
              .map((item) => (
                <div key={item.tempId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.05)", padding: "10px", marginBottom: "5px", borderRadius: "8px" }}>
                  ...
                  <button className="notranslate" onClick={() => onRemoveItem(item.tempId)} style={{ background: "#e74c3c", color: "white", border: "none", padding: "5px 10px", borderRadius: "5px", cursor: "pointer" }}>
                    ✕
                  </button>
                </div>
              ))}
        ...
        <div className="notranslate" style={{ marginTop: "20px", borderTop: `1px solid ${style?.text || "#ccc"}`, paddingTop: "20px" }}>
          {carrello.length > 0 && (canOrder || isStaffQui) && (
            <button
              onClick={onSendOrder}
              style={{ width: "100%", padding: "15px", fontSize: "18px", background: "#159709ff", color: "white", border: `1px solid ${style?.text || "#ccc"}`, borderRadius: "30px", fontWeight: "bold", cursor: "pointer" }}
            >
          ...
          <button
            onClick={onClose}
            style={{ width: "100%", padding: "15px", marginTop: "10px", background: "transparent", border: `1px solid ${style?.text || "#ccc"}`, color: style?.text || "#ccc", borderRadius: "30px", cursor: "pointer" }}
          >
