...
const TempControl = ({ 
  ...
  logs,
  openDownloadModal // Nuova prop
}) => {
  return (
    <div className="no-print">
      <div style={{marginBottom:20, display:'flex', justifyContent:'flex-end'}}>
          <button onClick={()=>openDownloadModal('temperature')} style={{background:'#27ae60', color:'white', border:'none', padding:'10px 20px', borderRadius:5, fontWeight:'bold', cursor:'pointer'}}>⬇ Scarica Report Temperature</button>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(350px, 1fr))', gap:20}}>
        {assetsToDisplay.map(asset => {
            ...
            // COPIA QUI IL CONTENUTO DEL MAP CHE TI AVEVO DATO PRIMA
            // O, PER BREVITÀ, RIMETTO SOLO LA PARTE DI RENDER PER EVITARE CONFUSIONE:
            ...
            if(asset.stato === 'spento') {
                return (
                    <div key={asset.id} style={{background:'#e0e0e0', padding:20, borderRadius:10, border:'2px solid #999', opacity:0.7, position:'relative'}}>
                        ...
                        <div style={{height:40, background:'#ccc', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', color:'#777', fontWeight:'bold', fontSize:12}}>NESSUNA RILEVAZIONE</div>
                    </div>
                );
            }
            ...
            return (
                <div key={asset.id} style={{background:'white', padding:15, borderRadius:10, boxShadow:'0 2px 5px rgba(0,0,0,0.1)', borderTop:'5px solid #bdc3c7'}}>
                     ...
                     <div style={{display:'flex', fontSize:'11px', fontWeight:'bold', color:'#7f8c8d', marginBottom:5}}>
                          ...
                          <div style={{width:160, display:'flex', justifyContent:'space-between'}}><span style={{width:50, textAlign:'center'}}>FOTO</span><span style={{width:100, textAlign:'center'}}>AZIONE</span></div>
                     </div>
                     <div style={{display:'flex', alignItems:'stretch', gap:10, height:45}}>
                        <input type="number" step="0.1" placeholder="°C" value={currentData.val || ''} onChange={e=>setTempInput({...tempInput, [asset.id]: {...currentData, val: e.target.value}})} style={{flex:1, borderRadius:5, border:'1px solid #ddd', fontSize:18, textAlign:'center', fontWeight:'bold'}} />
                        <div style={{display:'flex', gap:5}}>
                            <button onClick={()=>registraTemperatura(asset, true)} style={{width:50, background:'#95a5a6', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontWeight:'bold', fontSize:'11px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center'}}><span>OFF</span></button>
                            <label style={{width:50, cursor:'pointer', background: currentData.photo ? '#2ecc71' : '#f1f2f6', borderRadius:5, display:'flex', alignItems:'center', justifyContent:'center', border:'1px solid #ddd'}}>
                                <span style={{fontSize:'20px'}}>📷</span><input type="file" accept="image/*" onChange={(e)=>handleLogPhoto(e, asset.id)} style={{display:'none'}} />
                            </label>
                            <button onClick={()=>registraTemperatura(asset, false)} style={{width:60, background:'#2c3e50', color:'white', border:'none', borderRadius:5, cursor:'pointer', fontWeight:'bold', fontSize:'12px'}}>SALVA</button>
                        </div>
                     </div>
