// client/src/Menu.jsx - VERSIONE V60 MULTILINGUA (Funzionante)
import { useState, useEffect } from 'react';
import { useParams, useSearchParams } from 'react-router-dom';
import { dictionary, getContent } from './translations'; // <--- IMPORTA IL DIZIONARIO
...
function Menu() {
  ...
  const t = dictionary[lang]; // Shortcut per i testi fissi (es. {t.cart})
  ...
  const inviaOrdine = async () => {
      ...
      if(!confirm(`${t.confirm}?`)) return; // Uso traduzione
      ...
      const prodottiNormalizzati = carrello.map(p => ({
          id: p.id, nome: p.nome, prezzo: p.prezzo, // Qui inviamo il nome italiano all'ordine (per la cucina)
  ...
  const footerBtnStyle = { 
    ...
    boxSizing: 'border-box', width: '100%', maxWidth: '280px', padding: '12px 15px',
    borderRadius:'30px', cursor:'pointer', fontSize:'13px', fontWeight:'bold', 
    display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', margin: '0 auto' 
  };
...
<div style={{ paddingBottom: '10px', marginTop: '10px', width: '100%', maxWidth: '600px', margin: '0 auto' }}>
  {categorieUniche.map(catNome => {
      // TRADUZIONE CATEGORIA: Cerchiamo un piatto di questa categoria per recuperare le traduzioni
      const representative = menu.find(p => (p.categoria_nome || p.categoria) === catNome);
      const translatedCatName = getContent({ nome: catNome, traduzioni: representative?.categoria_traduzioni }, 'nome', lang);
    ...
    <div key={catNome} className="accordion-item" style={{ marginBottom: '2px', borderRadius: '5px', overflow: 'hidden', width: '100%' }}>
      <div onClick={() => toggleAccordion(catNome)} style={{ background: activeCategory === catNome ? 'rgba(0,0,0,0.4)' : 'rgba(255,255,255,0.1)', color: titleColor, padding: '15px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: activeCategory === catNome ? `1px solid ${priceColor}` : 'none' }}>
        <h2 style={{ margin: 0, fontSize: '18px', color: titleColor, width: '100%' }}>{translatedCatName || catNome}</h2>
              ...
              <div key={scKey} style={{ width: '100%' }}>
                {!isSingleGroup && (
                  <div onClick={() => toggleSubAccordion(scKey)} style={{ background: 'rgba(255,255,255,0.05)', borderLeft: `4px solid ${priceColor}`, padding: '10px', margin: '1px 0', width: '100%', boxSizing: 'border-box', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ margin: 0, fontSize: '16px', color: titleColor, textTransform: 'uppercase' }}>{scKey === "Generale" ? t.others : scKey}</h3>
                ...
                {(isSingleGroup || activeSubCategory === scKey) && (
                  <div className="menu-list" style={{ padding: '0', width: '100%' }}>
                    {sottoCats[scKey].map((prodotto) => {
                      ...
                      // TRADUZIONE PIATTO
                      const nomePiatto = getContent(prodotto, 'nome', lang);
                      const descPiatto = getContent(prodotto, 'descrizione', lang);
                      ...
                      return (
                        <div key={prodotto.id} className="card" onClick={() => prodotto.immagine_url ? apriModale(prodotto) : null} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '15px', padding: '10px', width: '100%', boxSizing: 'border-box', cursor: prodotto.immagine_url ? 'pointer' : 'default', backgroundColor: cardBg, borderBottom: `1px solid ${cardBorder}` }}>
                          ...
                          <div className="info" style={{ flex: 1 }}>
                            <h3 style={{ margin: '0 0 2px 0', fontSize: '16px', color: style.text || '#222', lineHeight: '1.2' }}>{nomePiatto}</h3>
                            {descPiatto && (<p style={{ fontSize: '12px', color: '#666', margin: '0 0 2px 0', lineHeight: '1.1' }}>{descPiatto}</p>)}
                            {ingStr && (<p style={{ fontSize: '11px', color: '#555', fontStyle: 'italic', margin: '0 0 2px 0', lineHeight: '1.1' }}><span style={{ fontWeight: 'bold' }}>{t.ingredients}:</span> {ingStr}</p>)}
                            {prodotto.allergeni && prodotto.allergeni.length > 0 && (
                              <div style={{ marginTop: '2px', display: 'flex', flexDirection: 'column', gap: '1px' }}>
                                {prodotto.allergeni.filter(a => !a.includes("❄️")).length > 0 && ( <div style={{ fontSize: '10px', color: '#e74c3c', fontWeight: 'bold', textTransform: 'uppercase' }}>⚠️ {t.allergens}: {prodotto.allergeni.filter(a => !a.includes("❄️")).join(', ')}</div> )}
                                {prodotto.allergeni.some(a => a.includes("❄️")) && ( <div style={{ fontSize: '10px', color: '#3498db', fontWeight: 'bold', textTransform: 'uppercase' }}>❄️ {t.frozen}</div> )}
                              </div>
                            )}
                            <div style={{ fontSize: '14px', fontWeight: 'bold', color: priceColor, marginTop: '2px' }}>{Number(prodotto.prezzo).toFixed(2)} €</div>
                          </div>
                          <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
                            {hasVar && ( <button onClick={(e) => { e.stopPropagation(); apriModale(prodotto); }} style={{ background: 'transparent', border: '1px solid #ccc', color: '#555', borderRadius: '5px', padding: '5px 8px', fontSize: '12px', cursor: 'pointer', fontWeight: 'bold' }}>{t.modify}✏️</button> )}
      ...
      {selectedPiatto && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.9)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding:'10px' }} onClick={() => setSelectedPiatto(null)}>
            {(() => {
                ...
                // Traduzioni Modale
                const nomePiattoModale = getContent(selectedPiatto, 'nome', lang);
                const descPiattoModale = getContent(selectedPiatto, 'descrizione', lang);
                ...
                <div style={{ background: modalBg, color: modalText, borderRadius: '10px', overflow: 'hidden', maxWidth: '600px', width: '100%', maxHeight:'95vh', overflowY:'auto', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', position:'relative', display:'flex', flexDirection:'column' }} onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{padding:'20px'}}>
                        <h2 style={{margin:'0 0 5px 0', fontSize:'1.8rem', color: '#000', fontWeight:'800'}}>{nomePiattoModale}</h2>
                        <p style={{color:'#666', fontSize:'1rem', lineHeight:'1.4'}}>{descPiattoModale}</p>
                    ...
                    <div style={{padding:'20px', background:'#f9f9f9', borderTop:'1px solid #ddd', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        ...
                        <button onClick={() => { aggiungiAlCarrello({ ...selectedPiatto, nome: selectedPiatto.nome, prezzo: prezzoFinale, varianti_scelte: tempVarianti }); }} style={{ background: priceColor, color: 'white', padding: '12px 25px', borderRadius: '30px', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: '1rem' }}>{canOrder ? t.add : "LISTA"}</button>
                    </div>
      ...
      {showCheckout && (
          <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background: style.checkout_bg || style.bg || '#222', color: style.checkout_text || style.text || 'white', display:'flex', flexDirection:'column', padding:'20px', overflowY:'auto' }}>
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px', borderBottom:`1px solid ${style?.text||'#ccc'}`, paddingBottom:'10px'}}>
                  <h2 style={{color: titleColor, margin:0}}>{canOrder ? `${t.summary} 📝` : `${t.order_list} 📝`}</h2>
              ...
              <div style={{flex:1, overflowY:'auto', maxWidth:'600px', margin:'0 auto', width:'100%'}}>
                  {carrello.length === 0 && <p style={{color: style?.text || '#fff', textAlign:'center'}}>{t.empty_cart}</p>}
                  {(() => {
                      ...
                      const nomePortata = [null, t.course_1, t.course_2, t.course_3, t.course_4]; // Mappatura nomi portate 
                      return coursePresenti.map((courseNum, index) => (
                          <div key={courseNum} style={{marginBottom:'25px'}}>
                              <h3 style={{ margin:'0 0 10px 0', color: coloriPortata[index] || '#ccc', borderBottom:`2px solid ${coloriPortata[index] || '#ccc'}`, display:'inline-block', paddingRight:20 }}>{nomePortata[courseNum] || `PORTATA ${courseNum}`}</h3>
                              {itemsCucina.filter(i => i.course === courseNum).map(item => {
                                  ...
                                  // NOTA: Qui nel carrello lasciamo il nome originale (italiano) o tradotto?
                                  // Solitamente lo staff vuole vedere l'italiano, ma il cliente vuole capire cosa ha ordinato.
                                  // Per ora mostriamo il nome con cui è stato aggiunto (che era tradotto).
                                  return (
                                    <div key={item.tempId} style={{background:'rgba(255,255,255,0.1)', borderRadius:10, padding:15, marginBottom:10, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                                        <div style={{flex: 1}}>
                                            ...
                                            {v.base && v.base.length > 0 && ( <div style={{fontSize:'11px', color:'#999', marginTop:'4px'}}>🧂 {t.ingredients}: {v.base.join(', ')}</div> )}
                                            ...
                                            <div style={{color: priceColor, fontSize:'0.9rem', marginTop: '8px', fontWeight: 'bold'}}>{Number(item.prezzo).toFixed(2)} €</div>
                                        </div>
