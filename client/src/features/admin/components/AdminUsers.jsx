...
function AdminUsers({ API_URL, user }) {
    ...
    // ✅ Socket refresh (mantiene nomi eventi attuali, niente polling)
    useEffect(() => {
        if (!socket || !user?.id) return;

        // entra nella stanza del ristorante (room = ristorante_id)
        try { joinRoom?.(user.id); } catch (e) {}

        const handleRefresh = () => ricaricaTutto();

        // eventi compatibili (se già esistono nel backend)
        socket.on('refresh_utenti', handleRefresh);
        socket.on('refresh_admin', handleRefresh);

        return () => {
            socket.off('refresh_utenti', handleRefresh);
            socket.off('refresh_admin', handleRefresh);
        };
    }, [socket, user?.id]);
        ...
        <div className="card" style={{ padding: '25px', background: 'white', borderRadius: '15px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
            ...
            <div style={{ borderBottom: '2px solid #eee', paddingBottom: '15px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                    {/* ✅ refresh manuale (fallback) */}
                    <button
                        onClick={ricaricaTutto}
                        style={{ padding: '10px 14px', borderRadius: '30px', border: '1px solid #ddd', cursor: 'pointer', fontWeight: 'bold', background: '#fff', color: '#2c3e50' }}
                        title="Aggiorna"
                    >
                        🔄
                    </button>
