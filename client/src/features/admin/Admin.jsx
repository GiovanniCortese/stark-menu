...
function Admin() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // ✅ FIX: supporta sia "user" (nuovo) che "stark_user" (vecchio)
      ...
      if (storedUserRaw) {
        try {
          ...
          // ✅ FIX: supporta sia "ruolo" che "role"
      ...
      try {
        ...
        if (data && data.id) {
          ...
          // Se l'utente nel localStorage non matcha il ristorante corrente
    ...
    fetch(`${API_URL}/api/categorie/${id}`)
      ...
      .then(data => {
        const sorted = Array.isArray(data)
          ? data.sort((a, b) => (a.ordine_visualizzazione || 0) - (b.ordine_visualizzazione || 0))
          : [];
        setCategorie(sorted);
      });
  };
  ...
  const ricaricaDati = () => {
    ...
    fetch(`${API_URL}/api/menu/${slug}`)
      .then(r => r.json())
      .then(d => { if (d.menu) setMenu(d.menu); });
  ...
  const handleLogout = () => {
    if (window.confirm("Uscire dal pannello?")) {
      ...
      // ✅ FIX: pulizia compatibile (nuovo + vecchio)
  ...
  if (config.account_attivo === false) {
    return (
      <div style={{
        display: 'flex', justifyContent: 'center', alignItems: 'center',
        minHeight: '100vh', flexDirection: 'column', padding: '20px', textAlign: 'center'
      }}>
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.sidebar}>
        ...
        {/* GESTIONE */}
        {ruolo !== 'editor' && <NavItem id="dashboard" icon="📊" label="Dashboard" />}
        {ruolo !== 'editor' && <NavItem id="prenotazioni" icon="📅" label="Prenotazioni" />}
        ...
        {ruolo !== 'editor' && <NavItem id="sala" icon="📐" label="Sala & Mappa" />}

        {/* Mostra gestione utenti se modulo attivo o se l'utente è admin/superadmin */}
        {(showUtenti || ruolo === 'admin' || ruolo === 'superadmin') && ruolo !== 'editor' && (
          <NavItem id="users" icon="👥" label="Staff" />
        )}

        {ruolo !== 'editor' && <NavItem id="security" icon="⚙️" label="Impostazioni" />}
        ...
        <div style={{ marginTop: 'auto', paddingTop: 20 }}>
          ...
          <button onClick={handleLogout} style={{
            ...
            border: 'none', padding: 10, borderRadius: 6, cursor: 'pointer'
          }}>
            🚪 Logout
          </button>
        </div>
      ...
      <div style={styles.main}>
        ...
        <header style={styles.header}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '0.8rem', color: '#888', textTransform: 'capitalize' }}>{ruolo}</div>
            </div>
            <div style={{
              width: 40, height: 40, borderRadius: '50%', background: '#ff9f43',
        ...
        <div style={styles.content}>
          ...
          <div style={{
            ...
            boxShadow: '0 2px 10px rgba(0,0,0,0.03)', minHeight: '100%'
          }}>

            {tab === 'dashboard' && ruolo !== 'editor' && (
